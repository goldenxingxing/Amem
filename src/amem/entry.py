...
class MemoryEntry(BaseModel):
    ...
    Used both for notes that last only as long as one conversation and for
    ...
    """Whether this outlives the conversation that produced it.

    Defaulted rather than required: this package only ever stores persistent
    entries, so a caller constructing one had to supply a value the package
    never varies. ``"session"`` is here for a host that keeps its own
    short-lived notes in the same shape and wants one type for both.
    """
    content: str
    ...
    *not* doing something, so "never force-push to main" and "no function names
    ...
    affirmed_at: float | None = None
    """When someone last confirmed this entry still stands, if they have.

    Consolidation offers pairs where a later entry looks like it replaced an
    earlier one, and the answer is often "no, both hold" — a later instruction
    usually adds to an earlier one rather than replacing it. Without somewhere
    to put that answer the pair is offered again next session, and again, which
    is how a prompt that asks for attention teaches people to withhold it.

    Deliberately not a permanent exemption. It is compared against the *other*
    entry's date, so affirming settles the question as it stood — and a genuinely
    newer entry arriving later reopens it, which is the case where the question
    ...
    key: str | None = None
    """Short semantic handle, e.g. ``ingest/repo-path``.
