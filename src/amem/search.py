...
def _connected(db_path: Path) -> Iterator[sqlite3.Connection]:
    """A connection that is actually closed when the block ends.

    ``with sqlite3.connect(...)`` commits or rolls back; it does not close.
    On POSIX the leak is invisible — the file can be unlinked while open — so
    it survived every local run and every Linux and macOS CI job. On Windows
    an open handle makes the file undeletable, which breaks the one thing this
    index promises: that it is a cache, and deleting it costs a rebuild.
    """
...
#:
#: The distinction that matters for retrieval is not which language a query is
#: in but whether its writing system marks word boundaries. A script that does
#: gets whole terms; one that does not has to be cut into character windows,
#: because there is nothing else to cut on.
#:
#: This used to be the Han block alone, under the name CJK, and the rest of the
#: expression that used it was ASCII — so Cyrillic, Arabic, Hangul, Greek,
#: Hebrew and Devanagari produced no searchable terms at all and every query in
#: them returned nothing, silently. Japanese written in kana did the same; the
#: Japanese that appeared to work was passing on its Han characters.
_UNSPACED = (
...
_CJK_RUN = re.compile(f"[{_UNSPACED}]+")

#: A word in a script that marks its own boundaries.
#:
#: ``\w`` minus the underscore and leading digits' worth of punctuation, with
#: the unspaced scripts removed so they take the other branch. Unicode-aware
#: rather than A-Z: the alphabet a caller writes in is not this package's to
#: choose.
#: A word must not swallow the unspaced script that follows it. Written as an
#: alternation because a negated class cannot both exclude those ranges and
#: admit the punctuation that belongs inside an identifier: the first version
#: kept `[\w._'-]*` as the tail, so "CodeGraph有什么限制" tokenised as one term
#: and Chinese recall fell two points on the same corpus.
_WORD = re.compile(rf"[^\W_{_UNSPACED}](?:[^\W{_UNSPACED}]|[.'-])*", re.UNICODE)

#: One or the other: a run of unspaced script, or a word.
_PIECE = re.compile(f"[{_UNSPACED}]+|" + _WORD.pattern, re.UNICODE)


def _entries_stamp(entries: Sequence[MemoryEntry]) -> str:
    """A cheap signature of which entries these are.

    Ids only: content changes are already caught per entry by the incremental
    sync, and hashing every body on every search would undo the reason that
    sync exists.
    """
    digest = 0
    for entry in entries:
        digest = (digest * 1_000_003 + hash(entry.id)) & 0xFFFFFFFFFFFF
    return f"{digest:012x}"
...
def _terms(query: str) -> list[str]:
    ...
    """
    out: list[str] = []
    for piece in _PIECE.findall(query):
...
class MemorySearchIndex:
    ...
    def search(
        ...
        if _CJK_RUN.search(query):
            ...
            if len(hits) >= limit:
                return hits
            tail = self._fts(query, entries, limit=limit) or []
            return _merge_tail(hits, tail, limit=limit)
        ...
        hits = self._fts(query, entries, limit=limit, expression=expression)
        if hits is None:
            # The index cannot serve any query on this machine. The scan is
            # language-agnostic and was already carrying the Chinese path, so
            # it carries this one too rather than the caller getting silence.
            return _scan(_needles(query) or [query], entries, limit=limit, cache=self._folded)
        return hits
    ...
    def _fts(
        ...
        *,
        ...
        """Index-side results, or ``None`` when the index cannot be used at all.

        The distinction is the point. An empty list means the index answered
        and had nothing; ``None`` means it could not answer, which is a
        different situation and has a different remedy — the scan. FTS5 with a
        ``trigram`` tokeniser needs SQLite 3.34 or newer, and a Python built
        against an older one, or one compiled without FTS5, fails here on every
        query. Returning ``[]`` for that made Latin searches report no matches
        forever, which reads exactly like an empty store.

        Failure never raises: search is an aid, and an unusable index should
        degrade, not break the tool call.
        """
        if expression is None:
            expression = _match_expression(query)
            if expression is None:
                return []
        try:
            with _connected(self._db_path) as db:
            ...
            self._note_index_unusable()
            return None
        return [SearchHit(*row) for row in rows]

    def _note_index_unusable(self) -> None:
        """Say once why the index is not being used, then stop saying it.

        Per query this would be noise; never saying it leaves a deployer
        wondering why results are thin on one machine and not another.
        """
        if self._warned_unusable:
            return
        self._warned_unusable = True
        logger.warning(
            "search index unusable (sqlite %s) — falling back to the scan for every query. "
            "FTS5 with a trigram tokeniser needs SQLite 3.34+; results are still correct "
            "but ranking is the scan's, and large stores will be slower.",
    ...
    def _sync(self, db: sqlite3.Connection, entries: Sequence[MemoryEntry]) -> None:
        ...
        one insert.
        """
        # The entries are part of the stamp, not only the file they came from.
        #
        # Keying on the source file alone assumes the caller always passes what
        # that file contains, which is true for the application this was
        # extracted from and is not a promise a library can make. Handed a
        # different list against an unchanged file, the index answered from the
        # previous one — the ids came back belonging to entries the caller had
        # never seen.
        stamp = f"{_fingerprint(self._source)}|{len(entries)}|{_entries_stamp(entries)}"
