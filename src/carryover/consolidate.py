...
#: reading "2026-05-06 起，采样率改用 EWMA，不再维护 48 小时窗口" is not merely
...
def _without_announcement(text: str) -> str:
    ...
    by negating what it replaces — "改用 EWMA，不再维护 48 小时窗口", "switched to
    exponential backoff, no longer uses a fixed interval" — and the negation
    guard then reads that as an inversion and refuses the pair. Two of three
    real revisions were lost to exactly this.
...
def find_superseded(entries: Sequence[MemoryEntry]) -> list[Supersession]:
    ...
    def settled(older: MemoryEntry, newer: MemoryEntry) -> bool:
        """Whether this pair has already been put to someone and kept.

        Compared against the newer entry's date rather than treated as a
        blanket exemption: the answer was about the store as it stood, and an
        entry written after it is a question that has not been asked.
        """
        return older.affirmed_at is not None and older.affirmed_at >= newer.created_at
...
#: Above this the advisory stops being something to mention when convenient.
#:
#: A softer instruction was measured doing nothing at all: a real store sat at
#: 83% of budget for weeks with a concrete list of superseded pairs in every
#: session's opening context, the handles and the command spelled out, and the
#: subject was never once raised. "When it fits" never fits — which is the
#: failure this project exists to fix, reappearing inside the mechanism meant
#: to prevent it.
#:
#: Set below the 83% that was observed failing, and above the 75% where the
#: list first appears: there should be room to mention it twice before it
#: becomes pressing, and room again after that before anything is lost.
PRESSURE_ACT_AT = 0.80
