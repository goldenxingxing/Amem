...
def main():
    ...
    print(f"{DATA.name}: {sum(len(t) for t, _ in data)} turns, {n_q} questions\n")
    print(f"{'system':<34}" + "".join(f"{'recall@' + str(k):>11}" for k in TOP_K))
    ...
    for label, fn in SYSTEMS.items():
        ...
        try:
            ...
            print(f"{label:<32}  failed: {type(exc).__name__}: {exc}"[:110])
            continue
        print(
            f"{label:<32}"
