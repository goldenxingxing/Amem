...
#:
#: Point ``TRANSCRIPTS_DIR`` at a tree of ``*/*.jsonl`` transcripts. Prefer
#: uncompacted ones: a summarised tail holds a handful of real user turns, which
#: is not enough to count anything. This is your own data, so there is no
#: default — the numbers in the README came from one person's history and are
#: reproducible only against a comparable one.
...
def _require(var: str) -> Path:
    """Fail loudly on an unset directory rather than silently measuring nothing.

    Guards the variable rather than the Path built from it: ``Path("")`` is
    ``Path(".")``, which is a real directory, so an unset variable would
    otherwise glob the working tree and report zero of everything.
    """
    raw = os.environ.get(var, "")
    if not raw or not Path(raw).is_dir():
        raise SystemExit(f"set {var} to a directory of your own transcripts")
    return Path(raw)
...
def user_turns() -> list[str]:
    ...
    out: list[str] = []
    for path in sorted(_require("TRANSCRIPTS_DIR").glob("*/*.jsonl")):
        try:
