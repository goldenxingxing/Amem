...
#: records) and session directories each holding a ``context.jsonl``. Both are
#: private data, so both are configuration rather than defaults.
...
async def build_store(client, sem) -> list[str]:
    ...
    replies = await asyncio.gather(
        *(
            ask(
                client,
                sem,
                EXTRACTION_PROMPT.format(conversation=conversation_tail(i), today=today),
            )
            for i in ids
        )
    )
    ...
    for reply, sid in zip(replies, ids, strict=True):
        facts += [c.content for c in parse_candidates(reply, session_id=sid)]
    return facts
