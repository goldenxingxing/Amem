...
class Model:
    ...
    def __init__(self, concurrency: int = 8, *, alt: bool = False) -> None:
        ...
        prefix = "BENCH_ALT" if alt else "BENCH"
        self._url = os.environ[f"{prefix}_BASE_URL"].rstrip("/") + "/chat/completions"
        self._key = os.environ["BENCH_ALT_API_KEY" if alt else "BENCH_API_KEY"]
        self._model = os.environ[f"{prefix}_MODEL"]
        # Some endpoints refuse `temperature: 0` outright — "only 1 is allowed
        # for this model" — and the failure arrives as an empty answer, which scores as
