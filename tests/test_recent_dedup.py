...
def test_appending_does_not_get_more_expensive_as_summaries_get_longer(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """The work per append must scale with how many summaries there are, not how long they are.

    On twenty summaries of a few thousand characters this path costs seconds if
    anything character-level is reintroduced, and it runs at session end.

    This used to be a stopwatch — under 200 ms — which measures the runner as
    much as the code, and duly failed on one Windows CI job at 495 ms while
    passing on two others running the same commit. Worse, it did not measure
    what its comment claimed: this path compares normalised text for equality
    and never calls the similarity matcher at all.
    ...
    def cost_at(multiplier: int) -> tuple[int, int]:
        ...
        path = tmp_path / f"recent-{multiplier}.jsonl"
        body = "the deploy runbook covers rollback and canary and escalation. " * (65 * multiplier)
        for index in range(DEFAULT_MAX_SUMMARIES):
            append_summary(path, summary(f"session-{index}", f"{index} {body}"))

        monkeypatch.setattr(dedup, "normalize_content", counting)
        append_summary(path, summary("session-new", f"new {body}"))
        monkeypatch.setattr(dedup, "normalize_content", real)
        return calls, len(body)

    short_calls, short_len = cost_at(1)
    long_calls, long_len = cost_at(4)

    assert short_calls > 0, "nothing was compared at all"
    assert long_len == short_len * 4
    assert long_calls == short_calls, (
        f"{long_calls} comparisons against {long_len}-character summaries versus "
        f"{short_calls} against {short_len}-character ones — the cost is following "
        "the text length, which is what character-level similarity does"
    )
