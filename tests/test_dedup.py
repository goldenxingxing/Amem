...
# One test did not come across: it asserted that this package's `fold_text`
# matches an authorization helper in the application it was extracted from.
# That is a constraint on the application — the two must not drift apart there
# — and asserting it here would make this library's tests fail for a reason
# outside it.
...
def test_long_text_is_compared_by_token_not_by_character() -> None:
    """The property, rather than a wall-clock number.

    `difflib` is quadratic in the length of both inputs, so comparing a long
    pair character by character costs enough to be felt on a write that checks
    against a whole file. `_matcher` switches to tokens past a threshold, and
    that is what keeps it affordable.

    Asserted as a ratio against the character-level cost rather than as a
    millisecond bound: an absolute number encodes an assumption about the
    machine, and fails on a loaded one for a reason that has nothing to do with
    the code.
    """
    import difflib
    ...
    character_level = time.perf_counter() - started

    assert score > AUTO_MERGE_RATIO
    assert ours < character_level / 4, (
        f"{ours * 1000:.0f}ms against {character_level * 1000:.0f}ms — "
        "long text is being compared character by character"
    )
