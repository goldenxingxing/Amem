...
Carried over from the application this package was extracted from, where the
rendering used to live. Overlaps test_inject.py in places and is kept whole
anyway: these were written independently and cover things that file does not —
that one section growing does not evict another, that behavioural memory
survives the cut, that the index dates what it lists.
"""
...
class TestHandles:
    def test_a_key_is_normalised_and_becomes_the_handle(self) -> None:
        e = _entry("project", "body", key="INGEST/Repo-Path")
        assert e.key == "ingest/repo-path"
        assert e.handle == "ingest/repo-path"
...
class TestKindSplit:
    def test_behaviour_is_stated_and_facts_are_indexed(self) -> None:
        ...
        entries = [
            ...
            _entry(
                "project",
                "ingest lives at /Users/x/ingest, not the copy under output/. "
        ...
        out = inject.render(entries, [])

        assert "Always verify the date before writing a report." in out
        assert "The user is an insulin pump manufacturer." in out
        assert "ingest lives at" in out, "the index still has to identify the fact"
        assert "TAIL-MARKER" not in out, "a long fact is summarised, not recited"
        assert "(ingest/repo, " in out and "(mail/config, " in out

    def test_the_index_says_how_to_read_an_entry(self) -> None:
        out = inject.render([_entry("project", "a fact", key="p/one")], [])
        # The action, not one host's spelling of it — see TestTheHostNamesItsOwnActions.
        assert inject.DEFAULT_ACTIONS.get in out
    ...
    def test_sections_are_omitted_when_empty(self) -> None:
        assert "Recorded facts" not in inject.render([_entry("feedback", "x")], [])
        assert "Persistent memory" not in inject.render([_entry("project", "x")], [])
...
class TestBudget:
    def test_an_oversized_index_says_how_much_of_itself_it_is_showing(self) -> None:
        ...
        assert len(out) <= inject._INDEX_BUDGET_CHARS + 700
    ...
    def test_one_section_growing_does_not_evict_another(self) -> None:
        ...
        out = inject.render(entries, recents)
...
class TestGroundTruth:
    def test_the_header_says_the_snapshot_is_established_fact(self) -> None:
        ...
        out = inject.render([_entry("feedback", "x")], [])
        lowered = out.lower()
...
class TestTruncationDirection:
    ...
    def test_a_section_heading_is_never_what_gets_cut(self) -> None:
        ...
        out = inject.render([_entry("project", "x" * 5000)], recents)
...
class TestSuggestions:
    def test_suggestions_are_marked_as_undecided(self) -> None:
        ...
        assert inject.DEFAULT_ACTIONS.promote in out

    def test_no_suggestions_means_no_section(self) -> None:
        assert "Suggested memories" not in inject.render([_entry("feedback", "x")], [], [])
...
class TestRetirement:
    ...
    def test_nothing_is_said_when_nothing_is_retired(self) -> None:
        out = inject.render([_entry("feedback", "NEVER force-push to main.")], [])

        assert "retired" not in out

    def test_a_retired_lookup_entry_leaves_the_index_too(self) -> None:
        kept = _entry("project", "ingest builds with uv")
        gone = _entry("project", "ingest used to build with poetry")
        gone.retired_at = 1_700_000_000.0

        out = inject.render([kept, gone], [])

        assert "ingest builds with uv" in out
...
class TestPressureWarnsWhileThereIsRoom:
    ...
    def test_a_store_with_room_is_left_alone(self) -> None:
        ...
        out = inject.render(self._entries(2, 300), [])
